...
class BaseDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        offset = 0
        for _index_file in range(self.num_files):
            index_file = os.path.join(data_path, "annotation/train", f"pair_{_index_file}.json")
        ...
        self.split = split
        self.nocrop = transforms.Compose([
        ...
        self.crop = transforms.Compose([
            RandomResizedCropAndInterpolation(384, scale=(0.5, 1.0), interpolation='bicubic'), 
    ...
    def _get_image(self, image_path: str):
        ...
        if torch.normal(mean=0.5, std=0.166667, size=(1,)).item() > 0.5:
            return self.crop(image)
        else:
            return self.nocrop(image)
    ...
    def _get_image_text_example(self, index: int, data: dict):
        ...
        tokens = self.tokenizer.tokenize(item["caption"])
        text_segment = self.tokenizer.convert_tokens_to_ids(tokens)
...
def create_downstream_dataset(args, is_eval=False):
    if is_eval:
        ...
        return \
            create_dataset_by_split(args, split="356", is_train=True), None
